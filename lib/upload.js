...
function simpleUpload( promise, read, options ){
	return promise.push([ gm( read ), options ]);
}

function argsUpload( promise, read, options, key, value1, value2 ){
	return promise.push([ gm( read )[ key ]( value1, value2 ), options ]);
}

function withoutArgsUpload( promise, read, options, key ){
	return promise.push([ gm( read )[ key ](), options ]);
}

function chainedUpload( promise, read, options, key1, key2, value1, value2 ){
	return promise.push([ gm( read )[ key1 ]( value1 )[ key2 ]( value2 ), options ]);
}
